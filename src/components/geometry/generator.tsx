...
const OwnGenerator: React.FC<States> = (state: States) => {
	...
	const selectAirfoil = async () => {
		try {
			...
			const response = await fetch("http://127.0.0.1:5000/naca/" + code + "/" + pts + "/txt");
			if (!response.ok) {
				throw new Error("Failed to fetch airfoils");
			}
			const airfoil_txt = await response.json();
			dispatch(geometrySetAirfoilName(a, airfoil_txt) as any)
		} catch (err: any) {
			// console.log(err.message);
		} finally {
			setLoading(false);
